...
class ContactStore extends ChangeNotifier {
  ...
  // BARU: memperbarui data kontak yang sudah ada (fitur Edit)
  ...
  // BARU: menghapus kontak dari daftar (fitur Hapus)
...
class _ContactCard extends StatelessWidget {
  ...
  // BARU: buka form Tambah/Edit Kontak dalam mode edit (data sudah terisi)
  void _bukaEdit(BuildContext context) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => TambahKontakScreen(contact: contact),
      ),
    );
  }

  // BARU: dialog konfirmasi sebelum kontak benar-benar dihapus
  void _konfirmasiHapus(BuildContext context) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Hapus Kontak'),
        content: Text('Yakin ingin menghapus "${contact.nama}"?'),
        actions: [
          TextButton(
          ...
          TextButton(
            onPressed: () {
              ContactStore.instance.remove(contact);
              Navigator.pop(ctx);
            },
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ListTile(
        ...
        isThreeLine: true,
        // BARU: tombol favorit, edit, dan hapus dalam satu baris
        trailing: Row(
...
// Jika `contact` diisi (mode edit), form terisi data lama dan
// tombol Simpan akan memperbarui data tersebut, bukan menambah baru.
// ==========================================================
class TambahKontakScreen extends StatefulWidget {
  final Contact? contact;

  const TambahKontakScreen({super.key, this.contact});
...
class _TambahKontakScreenState extends State<TambahKontakScreen> {
  ...
  void initState() {
    super.initState();
    // BARU: jika mode edit, isi field dengan data kontak yang sudah ada
    _namaController = TextEditingController(text: widget.contact?.nama ?? '');
    _emailController =
        TextEditingController(text: widget.contact?.email ?? '');
    _hpController = TextEditingController(text: widget.contact?.noHp ?? '');
    _kategoriController =
  ...
  void _simpanKontak() {
    if (_formKey.currentState!.validate()) {
      ...
      final hasilKontak = Contact(
        nama: _namaController.text.trim(),
        email: _emailController.text.trim(),
        noHp: _hpController.text.trim(),
        kategori: kategori.isEmpty ? null : kategori,
        // Status favorit lama tetap dipertahankan saat mode edit
        isFavorite: widget.contact?.isFavorite ?? false,
      );

      if (_isEdit) {
        ContactStore.instance.update(widget.contact!, hasilKontak);
      } else {
        ContactStore.instance.add(hasilKontak);
      }
      Navigator.pop(context);
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              ElevatedButton.icon(
                ...
                icon: Icon(_isEdit ? Icons.check : Icons.save_outlined),
